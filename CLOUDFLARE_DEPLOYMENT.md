...
| Actions Variable `CLOUDFLARE_PAGES_PROJECT` | Pages 项目名称，例如 `novel-forge-pages`    |
...
本次没有改写 DNS，也没有绑定具体域名，因为当前对话中还没有提供你申请的实际域名、Cloudflare Pages 项目名和账户 ID。完成 Pages 部署后，请在 Cloudflare 面板中绑定域名；域名生效后再将该域名补充到 OAuth 回调白名单和后端 CORS/Origin 允许列表。
