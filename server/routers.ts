...
import { getDb, getProject, getProjectDocs, getProjects, getChapters, getTrends, getTrendRefreshRuns, getNotifications, cleanupDraftBackups, chapters, contentVersions, generationUsage, novelProjects, notifications, projectDocs, trendTags, trendRefreshRuns, writingSchedules } from "./db";
...
export const appRouter = router({
  ...
  workspace: router({
    ...
    saveDraftBackup: protectedProcedure.input(z.object({ projectId: z.number(), entityType: z.enum(["outline", "chapter"]), entityId: z.number(), content: z.string().min(1).max(300000) })).mutation(async ({ ctx, input }) => { const db = await getDb(); if (!db) throw new Error("数据库不可用"); await requireEntity(ctx.user.id, input.projectId, input.entityType, input.entityId); await db.insert(contentVersions).values({ projectId: input.projectId, userId: ctx.user.id, entityType: input.entityType, entityId: input.entityId, label: `服务端草稿备份 · ${new Date().toISOString()}`, content: input.content }); return { success: true }; }),
    latestDraftBackup: protectedProcedure.input(z.object({ projectId: z.number(), entityType: z.enum(["outline", "chapter"]), entityId: z.number() })).query(async ({ ctx, input }) => { const db = await getDb(); if (!db) return null; await requireEntity(ctx.user.id, input.projectId, input.entityType, input.entityId); return (await db.select().from(contentVersions).where(and(eq(contentVersions.projectId, input.projectId), eq(contentVersions.userId, ctx.user.id), eq(contentVersions.entityType, input.entityType), eq(contentVersions.entityId, input.entityId), like(contentVersions.label, "服务端草稿备份 ·%"))).orderBy(desc(contentVersions.createdAt)).limit(1))[0] ?? null; }),
    cleanupDraftBackups: protectedProcedure.input(z.object({ projectId: z.number(), retentionDays: z.number().int().min(1).max(3650).default(30), keepLatest: z.number().int().min(1).max(100).default(10) })).mutation(async ({ ctx, input }) => { await requireProject(ctx.user.id, input.projectId); return cleanupDraftBackups(ctx.user.id, input.projectId, input); }),
