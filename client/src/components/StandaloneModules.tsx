...
export function StandaloneTrendPanel({ trends, runs = [], onRefresh, refreshing = false }: { trends: any[]; runs?: any[]; onRefresh?: () => void; refreshing?: boolean }) {
  const lastRun = runs[0];
  return <Card className="rounded-none motion-rise"><CardHeader><div className="flex flex-wrap items-start justify-between gap-4"><div><CardTitle className="font-display text-3xl">题材趋势库</CardTitle><p className="text-sm text-muted-foreground leading-6">把公开平台的题材观察和你的自定义标签放在同一张表里，方便按平台、题材和可信度比较。</p></div>{onRefresh && <Button type="button" variant="outline" className="rounded-none" disabled={refreshing} onClick={onRefresh}>{refreshing ? "研究中…" : lastRun?.status === "failed" ? "重试趋势研究" : "手动生成趋势建议"}</Button>}</div></CardHeader><CardContent><div className="mb-6 border-l-2 border-accent pl-4 text-xs leading-6 text-muted-foreground"><strong className="text-foreground">多平台公开观察 · 采集于 2026-08-13</strong><br />番茄记录公开分类与首页观察；抖音记录公开搜索主题；B站记录公开视频搜索结果。三者均不是实时全量榜单。<div className="mt-2 flex flex-wrap gap-x-4 gap-y-1">{MULTI_PLATFORM_TREND_SOURCES.map(source => <a key={source.platform} className="underline underline-offset-4" href={source.url} target="_blank" rel="noreferrer">查看 {source.platform} 来源</a>)}</div></div>{lastRun && <div className="mb-5 flex flex-wrap items-center justify-between gap-3 border border-border px-3 py-3 text-xs"><span>最近运行：{lastRun.status === "succeeded" ? `成功，生成 ${lastRun.itemCount} 条` : lastRun.status === "failed" ? "失败" : lastRun.status}</span><span className="text-muted-foreground">{new Date(lastRun.startedAt).toLocaleString()}</span></div>}<TrendTable trends={trends} /></CardContent></Card>;
}
