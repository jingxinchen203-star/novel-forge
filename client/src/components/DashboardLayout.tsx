...
import { BookOpen, Bell, CalendarClock, FileText, LayoutDashboard, LogOut, Moon, PanelLeft, Sun, Tags } from "lucide-react";
...
function DashboardLayoutContent({ children, setSidebarWidth }: { children: React.ReactNode; setSidebarWidth: (width: number) => void }) {
  const { user, logout } = useAuth(); const { theme, toggleTheme } = useTheme(); const { state, toggleSidebar } = useSidebar(); const isCollapsed = state === "collapsed"; const [resizing, setResizing] = useState(false); const [activeTarget, setActiveTarget] = useState(() => readNavTarget(currentHash())); const ref = useRef<HTMLDivElement>(null);
