...
function diffTokens(value: string) {
  return value.match(/[\\u4e00-\\u9fff]|[A-Za-z0-9_]+|[^\\s\\w]/g) ?? [];
}
...
function DiffHighlightedText({ original, revised, ariaLabel }: { original: string; revised: string; ariaLabel: string }) {
          ...
          <main className="min-w-0">
            ...
            {!selected ? (
              navigationTarget === "trends" ? (
                <StandaloneTrendPanel trends={trends.data ?? []} />
              ) : (
                <StandaloneModulePanel
            ...
            <CardHeader className="flex-row justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-2">
                  手动标签与多平台公开观察统一进入表格，可按平台、小说分类和可信度筛选。
                </p>
              </div>
              <Button type="button" variant="outline" className="rounded-none" disabled={refreshTrends.isPending} onClick={() => refreshTrends.mutate()}><Sparkles className="mr-2 h-4 w-4" />{refreshTrends.isPending ? "研究更新中…" : "手动更新趋势"}</Button>
            </CardHeader>
            <CardContent>
